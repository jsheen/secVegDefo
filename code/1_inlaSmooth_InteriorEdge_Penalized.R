...
age_range <- 3:33
# Generate the exact column names for Total and Edge by age
total_age_cols <- paste0("df_perc_area_secondary_prev_age_", age_range)
edge_age_cols <- paste0("df_perc_area_secondary_edge_prev_age_", age_range)
# Calculate row-wise sums across only those specific age columns
deforest_wide[, sum_total_sec_age3_33 := rowSums(.SD, na.rm = TRUE), .SDcols = total_age_cols]
deforest_wide[, sum_edge_sec_age3_33 := rowSums(.SD, na.rm = TRUE), .SDcols = edge_age_cols]
# Calculate the Proportion (handling zeroes)
deforest_wide[, Prop_Edge := fifelse(
  sum_total_sec_age3_33 == 0, 
  0, 
  sum_edge_sec_age3_33 / sum_total_sec_age3_33
)]

# # Test for upper triangular
# # 1. Define the boundary function
# # This check: For any given (year, mun_exp), age must be <= (year - 1986)
# # If a value exists for an age > (year - 1986), it is a FAILING row.
# test_strict_triangularity <- function(df, total_cols) {
#   # Extract numeric ages
#   col_ages <- as.numeric(gsub(".*_age_", "", total_cols))
#   # Find all invalid cells (Value is NOT NA, but Age > Year - 1986)
#   failures <- lapply(seq_along(total_cols), function(i) {
#     age <- col_ages[i]
#     col <- total_cols[i]
#     # Indices where it is physically impossible for forest to have that age
#     is_impossible <- (df$year - 1986) < age
#     # Find rows where there is an actual value in the "impossible" zone
#     bad_rows <- which(is_impossible & !is.na(df[[col]]))
#     if(length(bad_rows) > 0) {
#       return(data.frame(
#         mun_exp = df$mun_exp[bad_rows],
#         year = df$year[bad_rows],
#         age = age,
#         col_name = col
#       ))
#     }
...
#   if(is.null(failure_df)) {
#     return("PASS: No leakage found in the upper triangle.")
#   } else {
#     return(failure_df) # Returns the exact rows that failed
#   }
# }
# # Run it
# result <- test_strict_triangularity(deforest_wide, total_cols)
# # Output
# if(is.character(result)) {
#   print(result)
# } else {
#   print("FAILED: Data leakage detected in rows:")
#   print(head(result))
# }

# Get 1 SD change in base units (for reference/reporting)
...
for (input_state in amazon_states) {
  ...
  # Create the lookup from the actual shapefile used for the graph
  ...
  # CRITICAL: Check if any data rows failed to match the shapefile
  ...
  # Total Area Matrix (age_matrix)
  age_cols_names <- paste0("df_perc_area_secondary_prev_age_", age_range)
  age_matrix <- as.matrix(d1_full[, ..age_cols_names])
  
  # Track where data actually exists (Weights matrix) otherwise NA
  W <- !is.na(age_matrix) 
  
  # Create zero-filled copies for multiplication
  age_matrix_zeroed <- age_matrix
  age_matrix_zeroed[!W] <- 0
  
  # Calculate the raw spline projections (contains zeroes)
  age_smooth_matrix <- age_matrix_zeroed %*% B_4
  
  # Calculate the ACTUAL basis total weight available per row
  actual_basis_sum <- W %*% B_4
  
  # Put a 1 where 0 so don't divide by 0 (which is only 2003 Jan.)  
  #which(rowSums(actual_basis_sum == 0) == ncol(actual_basis_sum))
  #which(rowSums(df == 0) > 0)
  #View(d1_full[which(rowSums(is.na(age_matrix)) == ncol(age_matrix)),])
  ...
  if (state_abrev != 'TO' & state_abrev != 'AC') {
    m21_sub <- cases ~ 1 +
      ...
      # --- Penalized Spline (RW2) Block ---
      f(id.as1, as1, model = "rw2", values = 1:4, scale.model = TRUE) +
      f(id.as2, as2, copy = "id.as1") +
      f(id.as3, as3, copy = "id.as1") +
      f(id.as4, as4, copy = "id.as1") +
      # ------------------------------------
      Prop_Edge +
      scale(area) +
  ...
  # Run INLA
  if (state_abrev == 'All') {
    current_theta <- c(0.0735, 0.9101, 4.0173, 4.1108, 3.2437, 0.2992,
                       7.6104, 1.7277, -1.2767, 1.0841, 1.2483, 4.5757, 3.1480)
    ...
    m21_sub_results_sub <- inla(
      ...
      control.mode = list(theta = current_theta, restart = TRUE), 
      control.inla = list(
        tolerance = 1e-3,
        int.strategy = "eb"
      ),
  ...
  fwrite(m21_sub_summary, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_altInteriorEdge.csv'), row.names=T)
  
  if (state_abrev == 'All') {
    save(m21_sub_results_sub, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_altInteriorEdge.RData'))
  }
  
  # ========================================================
  # SAMPLING POSTERIOR (EXTRACTING BASE SPLINES + PROP_EDGE)
  # ========================================================
  # Draw 1,000 samples
  print("Extracting targeted fixed effects...")
  
  target_vars <- list(id.as1 = 1:4, Prop_Edge = 1) 
  ...
  # The rw2 nodes are named "id.as1:1", "id.as1:2", etc.
  latent_names_sample1 <- rownames(samples[[1]]$latent)
  match_indices <- numeric(5)
  
  for(j in 1:5) {
    if (j <= 4) {
      # Match the Random Walk nodes
      target_name <- paste0("^id\\.as1:", j, "$")
    } else {
      # Match the fixed effect (INLA sometimes appends :1 to fixed effects)
      target_name <- "^Prop_Edge(:1)?$" 
    }
    ...
    if(length(idx) == 1) {
      ...
      stop(paste("Could not uniquely find coefficient for index", j, "- searching for", target_name))
    }
  }
  
  # Extract the raw coefficients across all samples (5 rows)
  # Rows 1:4 = RW2 Spline nodes. Row 5 = Prop_Edge.
  beta_matrix_raw <- sapply(samples, function(s) s$latent[match_indices, 1])
  
  # Reconstruct the B-spline matrix for export
  age_range <- 3:33
  B_4 <- bs(age_range, df = 4, intercept = TRUE)
  B_4_scaled <- t(t(as.matrix(B_4)) / (as_sds * colMeans(actual_basis_sum_matrix, na.rm = TRUE)))
  
  sd_by_age_raw <- apply(d1_full[, ..total_age_cols], 2, sd, na.rm = TRUE)
  
  spline_outputs <- list(
    beta_matrix_raw = beta_matrix_raw, 
    as_sds = as_sds, 
    B_4_scaled = B_4_scaled, 
    sd_by_age_raw = sd_by_age_raw
  )
  
  save(spline_outputs, file=paste0('~/Desktop/secDef/mon_results/age_prof/smooth_', state_abrev, '_sub_altInteriorEdge.RData'))
}
