...
for (input_state in amazon_states) {
  ...
  # 4. Assign the indices for the formula
  ...
  age_smooth_matrix <- age_matrix_zeroed %*% B_5
  
  actual_basis_sum <- W %*% B_5
  ...
  d1_full$as5_raw <- as.numeric(age_smooth_matrix_adj[,5])
  ...
  d1_full$as5 <- (d1_full$as5_raw - as_means[5]) / as_sds[5] 
  ...
  fwrite(m21_sub_summary, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_5df.csv'), row.names=T)
  if (state_abrev == 'All') {
    save(m21_sub_results_sub, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_5df.RData'))
  }
  
  target_vars <- list(as1 = 1, as2 = 1, as3 = 1, as4 = 1, as5 = 1)
  ...
  spline_names <- c("as1", "as2", "as3", "as4", "as5")
  ...
  B_5_scaled <- t(t(as.matrix(B_5)) / as_sds)
  
  sd_by_age_raw <- apply(d1_full[, ..age_cols_names], 2, sd, na.rm = TRUE)
  
  spline_outputs <- list(beta_matrix_raw=beta_matrix_raw, as_sds=as_sds, B_5_scaled=B_5_scaled, sd_by_age_raw=sd_by_age_raw) 
  save(spline_outputs, file=paste0('~/Desktop/secDef/mon_results/age_prof/smooth_', state_abrev, '_sub_5df.RData')) 
}