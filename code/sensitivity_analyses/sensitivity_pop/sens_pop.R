...
mean_log_pop <- mean(log(deforest_wide$population))

# Define the exact age range for splines
age_range <- 3:33
...
for (input_state in amazon_states) {
  ...
  fwrite(m21_sub_summary, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_sensPopInteriorEdge.csv'), row.names=T)
  
  if (state_abrev == 'All') {
    save(m21_sub_results_sub, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_sensPopInteriorEdge.RData'))
  }
  ...
  # Pull the official coefficient names
  spline_names <- c("as1", "as2", "as3", "as4", "Prop_Edge")
  
  # Pre-calculate the exact row indices in the latent matrix
  ...
  # Rows 1:4 = as1 to as4. Row 5 = Prop_Edge.
  beta_matrix_raw <- sapply(samples, function(s) s$latent[match_indices, 1])
  
  # Reconstruct the B-spline matrix for export
  age_range <- 3:33
  B_4 <- bs(age_range, df = 4, intercept = TRUE)
  B_4_scaled <- t(t(as.matrix(B_4)) / (as_sds * colMeans(actual_basis_sum, na.rm = TRUE)))
  
  # Get raw empirical standard deviations for reference
  sd_by_age_raw <- apply(d1_full[, ..total_age_cols], 2, sd, na.rm = TRUE)
  
  # Save the outputs needed for ggplot
  spline_outputs <- list(
  ...
  save(spline_outputs, file=paste0('~/Desktop/secDef/mon_results/age_prof/smooth_', state_abrev, '_sub_sensPopInteriorEdge.RData'))
}
