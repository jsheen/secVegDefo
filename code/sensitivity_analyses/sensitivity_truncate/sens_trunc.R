...
deforest_wide[, sum_total_sec_age3_17 := rowSums(.SD, na.rm = TRUE), .SDcols = total_age_cols]
deforest_wide[, sum_edge_sec_age3_17 := rowSums(.SD, na.rm = TRUE), .SDcols = edge_age_cols]
# Calculate the Proportion (handling zeroes)
deforest_wide[, Prop_Edge := fifelse(
  sum_total_sec_age3_17 == 0, 
  0, 
  sum_edge_sec_age3_17 / sum_total_sec_age3_17
)]
#deforest_wide_save <- deforest_wide
#deforest_wide <- deforest_wide[which(deforest_wide$year >= 2010),]
length(which(is.na(deforest_wide$df_perc_area_secondary_edge_prev_age_17)))
length(which(is.na(deforest_wide$df_perc_area_secondary_edge_prev_age_18)))
...
for (input_state in amazon_states) {
  ...
  fwrite(m21_sub_summary, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_sensTruncInteriorEdge.csv'), row.names=T)
  
  if (state_abrev == 'All') {
    save(m21_sub_results_sub, file=paste0('~/Desktop/secDef/mon_results/', state_abrev, '_age_smooth_sub_sensTruncInteriorEdge.RData'))
  }
  ...
  save(spline_outputs, file=paste0('~/Desktop/secDef/mon_results/age_prof/smooth_', state_abrev, '_sub_sensTruncInteriorEdge.RData'))
}
